...
class DirectoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label_files is not None:
            ...
            if label.dim() == 3:  # Check if label has 3 dimensions (C, H, W)
                label = label.squeeze(0)  # Squeeze the channel dimension, resulting in (H, W)    
            label = (label > 0).long()  # Ensure label is binary (0 or 1)
        else:
